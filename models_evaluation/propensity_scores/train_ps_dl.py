...
class MyDataset(Dataset):
    def __init__(self, file_name, samples_type, pseudo=None):
        self.samples=pd.read_csv("../datasets/{}_X_{}.csv".format(file_name, samples_type), index_col=0).drop(['T', 'scores'], axis=1)
        self.labels=pd.read_csv("../datasets/{}_X_{}.csv".format(file_name, samples_type), index_col=0).loc[:,['T']]
...
def main(file_name):
    ...
    for cur_epoch in np.arange(n_epochs):
        ...
        if cur_epoch%MOD_FACTOR==0:
            with torch.no_grad():
                for i, data in enumerate(loader_test, 0):
                    x, y = data
                    y_hat = net.learn(x)
                    test_loss = nn.BCELoss()(y_hat, y)
                    running_loss_test += test_loss.item()

                running_loss_test /= (i + 1)
                l_running_loss_test.append(running_loss_test)
        ...
        if cur_epoch %MOD_FACTOR == 0:
            ...
            print "epoch#{}: loss test: {}".format(cur_epoch, running_loss_test)
            print ""

    end=time.time()
    torch.save(net.state_dict(), "../output/dl_ps_model_{}".format(file_name))
