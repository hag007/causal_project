...
def main(file_name):
    ...
    model=LinearRegression().fit(df_X_train, df_y_train)
    y_hat=model.predict(df_X_test)
    loss=np.sqrt(mean_squared_error(y_hat, df_y_test))
    file("../output/linear_regression_{}".format(file_name),'w+').write(pickle.dumps(model))
    print "loss_linear_regression: {}".format(loss)
