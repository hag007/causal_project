...
class p_x_z(nn.Module):
    ...
    def __init__(self, dim_in=20, nh=3, dim_h=20, dim_out_bin=19, dim_out_con=6):
        ...
        self.output_bin_mu = nn.Linear(dim_h, dim_out_bin)
        self.output_bin_sigma = nn.Linear(dim_h, dim_out_bin)
    ...
    def forward(self, z_input):
        ...
        # for binary outputs:
        x_bin_mu, x_bin_sigma = self.output_bin_mu(z), self.softplus(self.output_bin_sigma(z))
        x_bin = normal.Normal(x_bin_mu, x_bin_sigma)
...
class p_y_zt(nn.Module):
    ...
    def __init__(self, dim_in=20, nh=3, dim_h=20, dim_x=5, dim_out=1):
        super().__init__()
        # save required vars
        self.nh = nh
        self.dim_out = dim_out
        ...
        self.hidden_x0 = nn.ModuleList([nn.Linear(dim_h+dim_x, dim_h+dim_x) for _ in range(nh)])
        self.mu_t0 = nn.Linear(dim_h+dim_x, dim_out)

        self.input_t1 = nn.Linear(dim_in, dim_h)
        # loop through dimensions to create fully con. hidden layers, add params with ModuleList
        self.hidden_t1 = nn.ModuleList([nn.Linear(dim_h, dim_h) for _ in range(nh)])
        self.hidden_x1 = nn.ModuleList([nn.Linear(dim_h+dim_x, dim_h+dim_x) for _ in range(nh)])
        self.mu_t1 = nn.Linear(dim_h+dim_x, dim_out)
...
class q_z_tyx(nn.Module):
    ...
    def __init__(self, dim_in=233+1, nh=3, dim_h=20, dim_out=20):
        super().__init__()
